...
ex) %union - sementic value를 위한 공용체. C의 union과 동일.
    ...
    %token name - 가장 기본적인 토큰을 선언하는 방식.
                  yylex()에 자동으로 '토큰 code 번호 #define' 형태로 생성해줌.
...
* Recursive rules: non-terminal symbol로 시작해 위에서 아래로 분리하는 
                    Top-Down Approach 방식으로 수행됨.
                    이 때 자기자신이 Grammar요소에 하나씩 포함되어 있는 경우에 해당.
...
/* Epilogue: 시작할 때 parser implementation file에 복사됨. 
             ...
             따라서 변수 이름을 지을 때 이러한 규칙은 피해야 함. 
             이러한 미리 정의되어 있는 함수들이나 변수들을 C-language Interface라 함.
             ...
             ex) 
                (1) Function int yyparse(void): token을 읽고, action을 실행시키고,
                 ...
                 (2) yylex(): lexical analzer 함수로 input stream으로부터 들어온 토큰을
                              알아보고, parser에 넘기는 역할을 함. yylex를 사용하기 위해 
                              bison의 -d 옵션을 통해 헤더파일을 분리시켜 파일을 생성한 경우,
                              'name.tab.h'의 형태로 생성되며 lex파일에서 정의하여 bison과
                              lex간의 의사소통을 할 수 있음.

                              * yyparse가 yylex() 함수를 사용하는 방식: 
                              yylex는 #define형태로 생성된 Bison Declaration들의 토큰 코드번호를
                              반환하고, 0이나 음수는 파일의 끝을 의미함. (lex에 의해 만들어진 
                              lexer의 시작은 yylex()라는 함수를 호출하며 시작. 특정 패턴이 토큰에
                              매치했을 때 실행되는 c코드는 yylex()의 결과값을 호출함수에 리턴하는
                              return문 포함 가능)
                              
                              ex) 
                                lex_rules1 {return ADD;} - bison에서 정의한 ADD의 토큰 코드번호를 반환
                                lex_rules2 {return 10;} - int 리턴이 가능하지만 좋은 방식 아님

                  (3) yyerror(): 토큰을 읽어들일 때 syntax error을 감지. error 발생 시 yyparse에 의해 
                                 error가 발생한 토큰을 넘겨 받으며 yerror 호출.
